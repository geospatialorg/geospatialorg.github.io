...
<figure style={{ textAlign: "center" }}>
  <img src={useBaseUrl('/img/ghiduri/procesari-etl/limite-administrative/corectare-denumiri-uat.jpg')} alt="Exemplu: denumirea de intrare a UAT-urilor (stânga) și rezultatul final (dreapta)" />
...
Datele au fost puse la dispoziție pentru download prin protocolul HTTP. De asemenea, au fost expune prin intermediul serviciilor standardizate OGC. Stratul poate fi găsit sub numele `ro_admin_lau_polygon` în documentele getCapabilities pentru WMS și WFS.
...
# Activarea profilului Anaconda cu utilitare instalate
...
curl -u user:password -XPUT -H "Content-type: text/xml" -d "<featureType><abstract>Limitele unităților administrativ teritoriale din România (poligon). Actualizare 05.05.2025. Sursa ANCPI.</abstract><enabled>true</enabled></featureType>" http://localhost:8080/geoserver/rest/workspaces/geospatial/datastores/romania/featuretypes/ro_admin_lau_polygon
