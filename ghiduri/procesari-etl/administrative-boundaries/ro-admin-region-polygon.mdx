...
# Regiuni dezvoltare, România (poligon)

## Cod set de date

**`ro_admin_region_polygon`**

<figure style={{ textAlign: "center" }}>
  <img src={useBaseUrl('https://services.geo-spatial.org/data/thumbnails/administrative-boundaries/region/ro-admin-region-polygon.svg')} alt="Limite administrative - Regiuni dezvoltare, România (poligon)" />
</figure>

## Abordare practică

Datele au fost obținute prin prelucrarea stratului [Limite administrative - județe, România (poligon)](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/0cf21019-7f4c-41ce-b1cb-3cac94a62d2e). Acest lucru s-a realizat printr-o procedură automată de procesare cel folosește un script Bash (mediu *nix), script ce este rulat după fiecare actualizare a datelor de la ANCPI. Pentru pașii de procesare, scriptul utilizează mai multe biblioteci open source și utilitare de sistem: [GDAL](https://gdal.org), [mapshaper](https://mapshaper.org/), [curl](https://curl.se/), zip. Scriptul rezultat este prezentat în secțiunea [Script ETL](#script-etl) și accesibil pentru descărcare pe pagina [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_region_polygon.sh) a geo-spatial.org.

## Prelucrarea datelor

### Comasarea geometriilor (dissolve)

S-a realizat prin unirea/comasarea geometriilor stratului [Limite administrative - județe, România (poligon)](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/0cf21019-7f4c-41ce-b1cb-3cac94a62d2e) folosind codul unic al regiunilor de dezvoltare (obținut din datele INS).
...
Datele au fost documentate în [Catalogul geo-spatial.org](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/b12a299a-1d61-4cfe-9db1-b9cc056279c0) și puse la dispoziție pentru download/access prin protocolul HTTP și prin intermediul serviciilor standardizate OGC. Pentru mai multe detalii accesați [Catalogul](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/b12a299a-1d61-4cfe-9db1-b9cc056279c0) și/sau secțiunea <a href={useBaseUrl('descarcare/date/administrative-boundaries')}>Descărcare</a>. Definițiile folosite pentru simbolizarea WMS a datelor au fost documentate în secțiunea [Simbolizare SLD](#simbolizare-sld).

## Diagrama fluxului de lucru

<iframe
  src="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_region_polygon.json"
  width="100%"
  height="600"
...
<a
  href="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_region_polygon.json"
  target="_blank"
...
```bash title="publish_update_ro_admin_region_polygon.sh"
#!/usr/bin/zsh
...
```
Script accesibil și pe [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_admin_region_polygon.sh).
...
```
Fișier accesibil și pe [GitHub](https://github.com/geospatialorg/simbolizare/blob/main/sld/administrative-boundaries/ro_admin_region_polygon_labels.sld).
...
```
Fișier accesibil și pe [GitHub](https://github.com/geospatialorg/simbolizare/blob/main/sld/administrative-boundaries/ro_admin_region_polygon.sld).
