...
# Regiuni dezvoltare, România (linie, geometrie simplificată)

## Cod set de date

**`ro_admin_region_simplified_line`**

<figure style={{ textAlign: "center" }}>
  <img src={useBaseUrl('https://services.geo-spatial.org/data/thumbnails/administrative-boundaries/region/ro-admin-region-simplified-line.svg')} alt="Limite administrative - Regiuni dezvoltare, România (linie, geometrie simplificată)" />
</figure>

## Abordare practică

Datele au fost obținute prin prelucrarea stratului ["Limite administrative - regiuni dezvoltare, România (poligon, geometrie simplificată)](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/b1c716ad-f94e-46bf-804e-2fc53b64bb1c). Acest lucru s-a realizat printr-o procedură automată de procesare ce folosește un script Bash (mediu *nix), script ce este rulat după fiecare actualizare a datelor de la ANCPI. Pentru pașii de procesare, scriptul utilizează mai multe biblioteci open source și utilitare de sistem: [GDAL](https://gdal.org), [mapshaper](https://mapshaper.org/), [Shapely](https://shapely.readthedocs.io/en/stable/), [curl](https://curl.se/), zip. Scriptul rezultat este prezentat în secțiunea [Script ETL](#script-etl) și accesibil pentru descărcare pe pagina [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_region_line.sh) a geo-spatial.org.

## Prelucrarea datelor

### Conversia poligon în linie și adăugarea câmpurilor

Deoarece nu vorbim despre o simplă conversie poligon în linie, pentru aceastră operațiune s-a folosit o bibliotecă avansată pentru lucrul cu date vectoriale: [Shapely](https://shapely.readthedocs.io/en/stable/), în jurul căreia s-a scris un [script dedicat în Python](https://github.com/geospatialorg/ingestie-date/blob/main/convert_ro_admin_region_simplified_polygon_to_line.py). Cu ajutorul acesteia, conversia s-a făcut în așa fel încât să existe o singură linie comună între două poligoane vecine sau limitele exterioare. De asemenea, au fost adăugate următoarele câmpuri, extrase din stratul de intrare:
...
Datele au fost documentate în [Catalogul geo-spatial.org](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/16150f54-428f-4d82-8563-1ea5d9ce1876) și puse la dispoziție pentru download/access prin protocolul HTTP și prin intermediul serviciilor standardizate OGC. Pentru mai multe detalii accesați [Catalogul](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/16150f54-428f-4d82-8563-1ea5d9ce1876) și/sau secțiunea <a href={useBaseUrl('descarcare/date/administrative-boundaries')}>Descărcare</a>. Definițiile folosite pentru simbolizarea WMS a datelor au fost documentate în secțiunea [Simbolizare SLD](#simbolizare-sld).

## Diagrama fluxului de lucru

<iframe
  src="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_region_simplified_line.json"
  width="100%"
  height="600"
...
<a
  href="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_region_simplified_line.json"
  target="_blank"
...
##############################################################################################################
#🛠 Publicare/actualizare Regiuni dezvoltare, România (linie, geometrie simplificată)
##############################################################################################################
...
gs_layer_title="Limite administrative - Regiuni dezvoltare, România (linie, geometrie simplificată)"
...
gs_layer_keywords=("România" "regiuni de dezvoltare" "limite administrative" "vector" "linie" "geometrie simplificată")
gs_layer_metadata_link="https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/16150f54-428f-4d82-8563-1ea5d9ce1876"
gs_layer_style="administrative-boundaries:ro_admin_region_simplified_line"


#⚙️ Date
layer_name="ro_admin_region_simplified_line"

echo "
🛠 Procesare Regiuni dezvoltare, România (linie, geometrie simplificată)
"
...
if [ -f ${region_data_path}/${layer_name}.gpkg ]; then
 ...
 #💾 apelează script Python pentru conversie și crearea câmpurilor leftRegion, leftId, respectiv rightRegion și rightId
 echo "💾 apelează script Python pentru conversie"
 python convert_ro_admin_region_simplified_polygon_to_line.py
...
ogr2ogr -of Parquet -nlt MULTILINESTRING -nln ${layer_name} -dialect sqlite -sql "SELECT a.id AS id, * FROM ${layer_name} AS a" ${region_data_path}/${layer_name}.parquet ${region_data_path}/${layer_name}.gpkg
...
```
Script accesibil și pe [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_admin_region_simplified_line.sh).

```python title="convert_ro_admin_region_simplified_polygon_to_line.py"
import json
...
```
Fișier accesibil și pe [GitHub](https://github.com/geospatialorg/simbolizare/blob/main/sld/administrative-boundaries/ro_admin_region_simplified_line.sld).
