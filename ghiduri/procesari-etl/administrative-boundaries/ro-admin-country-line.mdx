...
# Frontiera de stat, România (linie)

## Cod set de date

**`ro_admin_country_line`**

<figure style={{ textAlign: "center" }}>
  <img src={useBaseUrl('https://services.geo-spatial.org/data/thumbnails/administrative-boundaries/country/ro-admin-country-line.svg')} alt="Limite administrative - Frontiera de stat, România (linie)" />
</figure>

## Abordare practică

Frontiera de stat a României, format vectorial/linie, a fost descărcată de pe [geoportalul ANCPI](https://geoportal.ancpi.ro/portal/apps/webappviewer/index.html?id=faeba2d173374445b1f13512bd477bb2) în format Shapefile. Ulterior, a fost trecută printr-o procedură automată de procesare folosind un script (Bash - mediu *nix) ce este rulat după fiecare actualizare a datelor de la ANCPI. Pentru pașii de procesare, scriptul utilizează mai multe biblioteci open source și utilitare de sistem: [GDAL](https://gdal.org), [mapshaper](https://mapshaper.org/), [curl](https://curl.se/), zip. Scriptul rezultat este prezentat în secțiunea [Script ETL](#script-etl) și accesibil pentru descărcare pe pagina [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_admin_country_line.sh) a geo-spatial.org.
...
<iframe
  src="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_country_line.json"
  width="100%"
  height="600"
...
<a
  href="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_country_line.json"
  target="_blank"
...
##############################################################################################################
#🛠 Publicare/actualizare Frontiera de stat, România (linie)
##############################################################################################################
...
echo "
🛠 Procesare Frontiera de stat, România (linie)
"
...
ogr2ogr -of GPKG -lco FID=id -a_srs "EPSG:3844" -nln ${layer_name} -dialect sqlite -sql "SELECT a.localId AS border, a.beginVers AS version, a.Geometry AS geometry FROM Limita_administrativa_tara AS a" ${country_data_path}/${layer_name}.gpkg ${ancpi_data_path}/Limita_administrativa_tara.shp
...
```
Script accesibil și pe [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_admin_country_line.sh).

## Simbolizare SLD

### Simbolizare SLD implicită

```xml title="ro_admin_country_line.sld"
...
```
Fișier accesibil și pe [GitHub](https://github.com/geospatialorg/simbolizare/blob/main/sld/administrative-boundaries/ro_admin_country_line.sld).
