...
# Macroregiuni, România (linie)

## Cod set de date

**`ro_admin_macroregion_line`**

<figure style={{ textAlign: "center" }}>
  <img src={useBaseUrl('https://services.geo-spatial.org/data/thumbnails/administrative-boundaries/macroregion/ro-admin-macroregion-line.svg')} alt="Limite administrative - Macroregiuni, România (linie)" />
</figure>

## Abordare practică

Datele au fost obținute prin prelucrarea stratului [Limite administrative - macroregiuni, România (poligon)](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/168c15e4-6774-409b-a0bc-06e06cd40717). Acest lucru s-a realizat printr-o procedură automată de procesare ce folosește un script Bash (mediu *nix), script ce este rulat după fiecare actualizare a datelor de la ANCPI. Pentru pașii de procesare, scriptul utilizează mai multe biblioteci open source și utilitare de sistem: [GDAL](https://gdal.org), [mapshaper](https://mapshaper.org/), [Shapely](https://shapely.readthedocs.io/en/stable/), [curl](https://curl.se/), zip. Scriptul rezultat este prezentat în secțiunea [Script ETL](#script-etl) și accesibil pentru descărcare pe pagina [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_macroregion_line.sh) a geo-spatial.org.

## Prelucrarea datelor

### Conversia poligon în linie și adăugarea câmpurilor

Deoarece nu vorbim despre o simplă conversie poligon în linie, pentru aceastră operațiune s-a folosit o bibliotecă avansată pentru lucrul cu date vectoriale: [Shapely](https://shapely.readthedocs.io/en/stable/), în jurul căreia s-a scris un [script dedicat în Python](https://github.com/geospatialorg/ingestie-date/blob/main/convert_ro_admin_macroregion_polygon_to_line.py). Cu ajutorul acesteia, conversia s-a făcut în așa fel încât să existe o singură linie comună între două poligoane vecine sau limitele exterioare. De asemenea, au fost adăugate următoarele câmpuri, extrase din stratul de intrare:

* `leftId` - ID-ul macroregiunii vecine 1;
* `rightId` - ID-ul macroregiunii vecine 2;
* `leftMacroregion` - numele macroregiunii vecine 1;
* `rightMacroregion` - numele macroregiunii vecine 2;
...
Datele au fost documentate în [Catalogul geo-spatial.org](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/fbed201c-fb51-4327-b333-ef0459c6d7ee) și puse la dispoziție pentru download/access prin protocolul HTTP și prin intermediul serviciilor standardizate OGC. Pentru mai multe detalii accesați [Catalogul](https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/fbed201c-fb51-4327-b333-ef0459c6d7ee) și/sau secțiunea <a href={useBaseUrl('descarcare/date/administrative-boundaries')}>Descărcare</a>. Definițiile folosite pentru simbolizarea WMS a datelor au fost documentate în secțiunea [Simbolizare SLD](#simbolizare-sld).

## Diagrama fluxului de lucru

<iframe
  src="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_macroregion_line.json"
  width="100%"
  height="600"
...
<a
  href="https://geo-spatial.org/jsoncrack/editor/?json=https://geo-spatial.org/descrieri-etl-json/etl_ro_admin_macroregion_line.json"
  target="_blank"
...
```bash title="publish_update_ro_admin_macroregion_line.sh"
#!/usr/bin/zsh

##############################################################################################################
#🛠 Publicare/actualizare Macroregiuni, România (linie)
##############################################################################################################
...
gs_layer_title="Limite administrative - Macroregiuni, România (linie)"
...
gs_layer_keywords=("România" "macroregiuni" "limite administrative" "vector" "linie")
gs_layer_metadata_link="https://services.geo-spatial.org/geonetwork/srv/eng/catalog.search#/metadata/fbed201c-fb51-4327-b333-ef0459c6d7ee"
gs_layer_style="administrative-boundaries:ro_admin_macroregion_line"


#⚙️ Date
layer_name="ro_admin_macroregion_line"
...
python convert_ro_admin_macroregion_polygon_to_line.py
...
rm ${macroregion_data_path}/${layer_name}.dbf ${macroregion_data_path}/${layer_name}.shp ${macroregion_data_path}/${layer_name}.prj ${macroregion_data_path}/${layer_name}.shx ${macroregion_data_path}/${layer_name}.cpg ${macroregion_data_path}/ro_admin_macroregion_line_tmp.gpkg
```
Script accesibil și pe [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/publish_update_ro_admin_macroregion_line.sh).

```python title="convert_ro_admin_macroregion_polygon_to_line.py"
import json
...
# === CONFIGURARE ===
input_file = "/storage/volumes/geoserver-1-storage/administrative_boundaries/macroregion/ro_admin_macroregion_polygon.gpkg"
input_layer_name = "ro_admin_macroregion_polygon"
field_name_name = "name"
field_name_id = "id"
output_file = "/storage/volumes/geoserver-1-storage/administrative_boundaries/macroregion/ro_admin_macroregion_line_tmp.gpkg"
output_layer_name = "ro_admin_macroregion_line_tmp"
...
```
Script accesibil și pe [GitHub](https://github.com/geospatialorg/ingestie-date/blob/main/convert_ro_admin_macroregion_polygon_to_line.py).

## Simbolizare SLD

### Simbolizare SLD implicită

```xml title="ro_admin_macroregion_line.sld"
...
```
Fișier accesibil și pe [GitHub](https://github.com/geospatialorg/simbolizare/blob/main/sld/administrative-boundaries/ro_admin_macroregion_line.sld).
